...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
    ...
    for(RouteModel::Node* node : current_node->neighbors) {
        ...
        // Calculate cost value as cumiliative sum
...
// Callback function for cell sort
...
// Cell sort algorithm to sort the vector of objects based on an attribute
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    // Exit loop when a Node with no parent is found (Start node)
...
void RoutePlanner::AStarSearch() {
    ...
    while(open_list.size() > 0) {
        ...
        // if search has reached the end_node, return the final path that was found
